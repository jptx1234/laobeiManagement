...
<form id="recipeForm" action="${pageContext.request.contextPath }/purchase/addPurchase.do" method="post">
	<div>
	...
	for(var i = 0; i < itemsOfType.length; i++){
		if(itemsOfType[i]["id"]==itemId){
			if(itemsOfType[i].hasOwnProperty("unit")){
				tds.find(":").text(itemsOfType[i]["unit"]);
			}else{
				$(tds[1]).text("份");
			}
...
function save(){
	var canSubmit = true;
	$(".has-error").removeClass("has-error").removeAttr("data-toggle").removeAttr("data-placement").removeAttr();
	$(":text").tooltip('destroy');
	$(".tooltip").remove();
	var patrn = /^\d+(\.\d+)?$/;
	$(":text[data-type='count']").each(function(){
